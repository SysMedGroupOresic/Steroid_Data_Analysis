...
CreateSankeyPlots <- function(RunAma, date, sick, Group, d) {
  ...
  # Assign color only to Steroids nodes
  # Combine steroid names with ACME values
  # Add ACME values to hoi
  hoi$ACME <- RunAma$ACME
  
  # Aggregate ACME per steroid and assign color
  steroid_colors <- aggregate(ACME ~ Steroids, data = hoi, FUN = mean)
  steroid_colors$color <- ifelse(steroid_colors$ACME < 0, "red", "blue")
  colnames(steroid_colors)[1] <- "node"
  
  
  
  
  
  # Merge color info into df2
  df2 <- left_join(df2, steroid_colors, by = "node")
  
  # Default color for non-steroid nodes
  df2$color[is.na(df2$color)] <- "grey80"
  
  # Plot
  p <- ggplot(df2, aes(x = x, next_x = next_x, node = node, next_node = next_node,
                       fill = I(color), label = node)) +
    geom_sankey(flow.alpha = 1, node.color = 1) +
    geom_sankey_label(size = 8.0, color = 1, fill = "white") +
    theme_sankey(base_size = 28) +
    theme(legend.position = "none") +
