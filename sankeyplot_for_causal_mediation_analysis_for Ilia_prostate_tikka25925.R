...
# all_all=read_xlsx(path = "C:/Users/patati/Documents/GitHub/Steroid_Data_Analysis/Basic Mediation Analysis for ProstateCancer_gleason_boot1000_adjusted_IliaS_tikka25925__All__mediation_results.xlsx")
all_all=read_xlsx(path = "C:/Users/patati/Documents/GitHub/Steroid_Data_Analysis/Basic CMA for ECs-Conditions-Lipid Selection_tikka25925 .xlsx")

#Basic CMA for ECs-Conditions-Lipid Selection_tikka25925 .xlsx
# Basic Mediation Analysis for ProstateCancer_gleason_boot1000_IliaS_tikka25925__All__mediation_results.xlsx
# Basic Mediation Analysis for ProstateCancer_gleason_boot1000_adjusted_IliaS_tikka25925__All__mediation_results.xlsx
...
lkm=15;Group='All'; name='just alal_uus';date=paste0(date,'_allds')#dim(all_all)[1];
...
rt2 <- rt2 %>%
  mutate(...1 = gsub("cancer_gleason_numeric", "Gleason", ...1))
...
hoi <- as.data.frame(matrix(hoi, ncol = 3, byrow = TRUE), stringsAsFactors = FALSE)
colnames(hoi) <- c('Endocannabinoids', 'Conditions', 'Lipids')
...
colnames(hoi) <- c("Contaminants", "Steroids", "Gleason")
# colnames(hoi) <- c('Endocannabinoids', 'Conditions', 'Lipids')
...
hoi$Contaminants <- gsub("L.PFHpS", "L-PFHpS", hoi$Contaminants)
# hoi$Steroids <- gsub("17a\\.OHP5", "17a-OHP5", hoi$Steroids)
hoi$Steroids <- gsub("17aOH\\.P4", "17a-OHP4", hoi$Steroids)
# hoi$Steroids <- gsub("11\\.KT", "11-KT", hoi$Steroids)
hoi$Steroids <- gsub("11\\.KA4", "11-KA4", hoi$Steroids)
hoi$Steroids <- gsub("11\\.KDHT", "11-KDHT", hoi$Steroids)
hoi$Steroids <- gsub("11b\\.OHA4", "11b-OHA4", hoi$Steroids)
hoi$Steroids <- gsub("T\\.Epi\\.T", "T/Epi-T", hoi$Steroids)


hoi$Lipids <- gsub("Cer_d18_24", "Cer(d18:1/24:1)", hoi$Lipids)
hoi$Lipids <- gsub("LPC_22", "LPC(22:5)", hoi$Lipids)
hoi$Lipids <- gsub("PC_38", "PC(38:4)", hoi$Lipids)
hoi$Lipids <- gsub("PC_40", "PC(40:5)", hoi$Lipids)
hoi$Lipids <- gsub("PC_o_32", "PC(O-32:0)", hoi$Lipids)
...
p=ggplot(hoi,
       aes(axis1 = Endocannabinoids, axis2 = Conditions, axis3 = Lipids,
           y = abs(ACME), fill = color)) +
  geom_alluvium(
    aes(fill = color,
  ...
  geom_text(stat = "stratum", aes(label = after_stat(stratum)),
            size = 6, fontface = 'bold') +
  scale_x_discrete(limits = c("Endocannabinoids", "Conditions", "Lipids"),
                   expand = c(.05, .05)) +
  scale_fill_manual(
    name   = "ACME Direction",
    values = c("blue" = "blue", "orange" = "orange"),
...
pngfile <- fs::path(path, 'sankey med res nonadj.png')

library(fs)
pngfile <- fs::path(fs::path_wd(), "sankey med res nonadj.png")



ragg::agg_png(pngfile, width = 150, height = 60, units = "cm", res = 500, scaling = 2)
print(p)
invisible(dev.off())
p
