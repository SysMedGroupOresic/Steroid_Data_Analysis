library(readxl)
library(ggplot2)
library(ggh4x)    # per-facet position scales, strip theming, panel sizing
library(grid)
library(magick)
...
build_fixed_orders <- function(groups2, global_y_order = NULL) {
  ...
  # Global Abbreviation_old order (if not supplied, use groups2's first-appearance order)
  ...
  list(group_levels = fixed_group_levels,
...
CalculateErrors <- function(
    ...
    use_qval = FALSE,              # colour by Q-values if TRUE
    alpha = 0.10,                  # significance cutoff
    se_multiplier = 0.5,           # ±(SE/2) error bars
    strict_gender = TRUE,          # require *_both/*_male/*_female
    fixed_group_levels = NULL,     # from build_fixed_orders(); if NULL, derived from groups2
    fixed_y_by_group   = NULL,     # from build_fixed_orders(); if NULL, derived from groups2
    global_y_order     = NULL,     # OPTIONAL: ensure a specific global order (vector of AbbrevOld)
    ...
    panel_row_height_mm = 10,      # <-- make each facet row a bit "wider" (taller)
    device_width_px = 8000,        # output width (tweak if you want more horizontal space)
    device_height_px = 11000,      # output height
    xlim = NULL,
    output_dir = getwd(),
    save_outputs = TRUE,
    return_plot = FALSE
  ...
  # ---- fixed orders (derive if missing); enforce global order if provided ----
  if (is.null(fixed_group_levels) || is.null(fixed_y_by_group)) {
    ...
    # Rebuild per-group lists from supplied global order
  ...
  # ---- expand per facet so y-axis always shows full set; add display names ----
  ...
  strip_arg <- ggh4x::strip_themed(
    size = "constant",  # stable strip margin sizing
    ...
    text_y       = ggh4x::elem_list_text(
      ...
      face   = "bold",
      hjust  = 0.5,  # <-- center horizontally after rotation
      vjust  = 0.5   # <-- center vertically after rotation
    )
  )
  
  # ---- per-facet y-scale limits (exact Abbreviation_old order) ----
  ...
  p <- ggplot2::ggplot(
    ...
    # Make each facet row a bit taller for readability
    ggh4x::force_panelsizes(
      rows = grid::unit(rep(panel_row_height_mm, length(fixed_group_levels)), "mm"),
    ...
    ggplot2::theme(
      ...
      strip.text.y.left   = ggplot2::element_text(
        angle = 90, hjust = 0.5, vjust = 0.5, margin = ggplot2::margin(r = 6)
      )
    )
  ...
  if (isTRUE(save_outputs)) {
    ...
    jpeg(jpg_path, width = device_width_px, height = device_height_px, quality = 100, pointsize = 16, res = 1000)
...
run_all_combinations <- function(
    ...
    global_y_order = NULL,          # <-- set your global order here
    use_qval = FALSE,
    alpha = 0.10,
    se_multiplier = 0.5,
    strict_gender = TRUE,
    ...
    panel_row_height_mm = 10,       # <-- make facet rows taller
    device_width_px = 8000,
    device_height_px = 11000,
    output_dir = getwd(),
    save_outputs = TRUE,
    return_plots = FALSE,
    verbose = TRUE
  ...
  # Compute fixed orders once (using global_y_order if given)
  ord <- build_fixed_orders(groups2, global_y_order = global_y_order)
  fixed_group_levels <- ord$group_levels
  fixed_y_by_group   <- ord$group_to_abbr
  global_y_order     <- ord$global_y_order
    ...
    for (cond in conditions) {
      ...
      out <- tryCatch({
        CalculateErrors(
          ...
          panel_row_height_mm = panel_row_height_mm,
...
log_df <- run_all_combinations(
  ...
  conditions = c("Steatosis","Fibrosis","Necroinflammation","HOMA-IR"),
  name_prefix = "Forest plot of",
  global_y_order = global_order,          # <-- lock the exact steroid order
  use_qval = FALSE,
  alpha = 0.10,
  se_multiplier = 0.5,
  strict_gender = TRUE,
  group_strip_fills = my_strip_fills,     # or NULL for defaults
  shorten_long_group_names = TRUE,
  panel_row_height_mm = 12,               # <-- make rows a bit taller than default
  device_width_px = 8500,                 # optional: a little wider horizontally
  device_height_px = 11000,
  output_dir = outdir,
  save_outputs = TRUE,
  return_plots = FALSE,
