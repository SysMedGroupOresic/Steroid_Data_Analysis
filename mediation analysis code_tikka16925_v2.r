...
# colnames(AllData)[colnames(AllData)=='17aOH-P4'] ='17a-OHP4'
  ...
  # Define covariates based on group
  if (Group %in% c("female", "male")) {
    covariates <- c("AGE", "BMI")
  } else {
    covariates <- c("AGE", "BMI", "Gender")
  }
  ...
  # OutcomeVariables_pruned <- sub("_L$", "", OutcomeVariables)
  # OutcomeVariables_pruned[OutcomeVariables_pruned=='C4Data'] ='C4'
  Y <- tv_red[, OutcomeVariables, drop = FALSE]
  
  
  # Ensure covariates exist in the dataset
  covariates <- covariates[covariates %in% colnames(tv_red)]
  
  # Combine all required variables
  Data <- na.omit(data.frame(cbind(X, M, Y, tv_red[, covariates, drop = FALSE])))
  ...
  for (y in y_names) {
    for (m in m_names) {
      for (x in x_names) {
        ...
        if (treat_val == control_val) {
          warning(paste("Skipping due to identical treat/control values for:", x))
          next
        }
        
        
        if (var(Data[[m]]) == 0 || var(Data[[y]]) == 0) {
          warning(paste("Skipping due to zero variance in mediator or outcome:", m, y))
          next
        }
        ...
        fmla1 <- as.formula(paste0("`", m, "` ~ `", x, "` + ", paste(covariates, collapse = " + ")))
        fmla2 <- as.formula(paste0("`", y, "` ~ `", x, "` + `", m, "` + ", paste(covariates, collapse = " + ")))
        ...
        if (any(is.na(coef(model_m))) || any(is.na(coef(model_y)))) {
          warning(paste("Skipping due to NA coefficients in models for:", x, m, y))
          next
        }
...
# Save to Excel
fn=''
file_path <- paste0("C:/Users/patati/Documents/GitHub/Steroid_Data_Analysis/", name, "_", fn, "_", Group, "_", date, "_mediation_results_mimmax_cov.xlsx")
...
cond <- if (Group == "female") {
  AllData[, "Gender"] == min(AllData[, "Gender"], na.rm = TRUE)
} else if (Group == "male") {
  AllData[, "Gender"] == max(AllData[, "Gender"], na.rm = TRUE)
} else {
  rep(TRUE, nrow(AllData))
}

# Apply sickness filter
tv_red <- if (sick == "yes") {
  AllData[cond & as.vector(sick_group), ]
} else {
  AllData[cond, ]
}

# Define covariates
covariates <- if (Group %in% c("female", "male")) {
  c("AGE", "BMI")
} else {
  c("AGE", "BMI", "Gender")
}
...
TreatmentVariables;MediatorVariables;OutcomeVariables
...
for (y in y_names) {
  for (m in m_names) {
    for (x in x_names) {
      ...
      if (var(Data[[m]]) == 0 || var(Data[[y]]) == 0) next
      ...
      if (any(is.na(coef(model_m))) || any(is.na(coef(model_y)))) next
      
      # Determine treatment/control values
      treat_val <- treat.value[x]
      control_val <- control.value[x]
      if (treat_val == control_val) next
      
      # Run mediation
      med_out <- tryCatch({
        mediation::mediate(model_m, model_y, treat = x, mediator = m,
                           sims = simss, control.value = control_val, treat.value = treat_val)
...
file_path <- paste0("C:/Users/patati/Documents/GitHub/Steroid_Data_Analysis/", name, "_", fn, "_", Group, "_", date, "_mediation_results_mimmax_cov3.xlsx")
cat("Saving results to:", file_path, "\n")
write.xlsx(res, file = file_path, append = FALSE, row.names = TRUE)
